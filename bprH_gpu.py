...
class bprH(object):
    ...
    def online_updating_user(self,
        ...
        self.I_u_t_train[u] = self.I_u_t_train[u].union(
            set(new_user_with_data[new_user_with_data.Action == 'P'].ItemID)) if u_already_exist else set(
            new_user_with_data[new_user_with_data.Action == 'P'].ItemID)
        self.I_u_a_train[u] = self.I_u_a_train[u].union(
            set(new_user_with_data[new_user_with_data.Action == 'V'].ItemID)) if u_already_exist else set(
            new_user_with_data[new_user_with_data.Action == 'V'].ItemID)
        ...
        while not converge_flag:
            ...
            if 0 <= indicator_sum <= 1:
                ...
                if indicator_J == 1:
                    ...
                    regular = self.lambda_u * cupy.linalg.norm(U_u, ord=2) + \
                              self.lambda_v * (cupy.linalg.norm(V_bar_I, ord=2) + cupy.linalg.norm(V_bar_K, ord=2)) + \
            ...
            ratio_delta_loss = abs(new_loss_u - loss_u ) / loss_u
            loss_u = new_loss_u
            if ratio_delta_loss < convergence_ratio_threshold:
                converge_flag = True
            # update estimation
            # old_estimation = self.estimation.copy()
            # self.estimation = cupy.dot(self.U, self.V)
